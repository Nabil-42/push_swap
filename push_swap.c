...
/*   Updated: 2024/03/18 15:37:10 by nabil            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	push_swap(t_args *args, char **argv, int argc)
{
	...
	args->count_a = argc - 1;
	args->count_b = 0;
	args->stack_a = (int *)malloc(sizeof(int) * (argc) + 2);
	args->stack_b = (int *)malloc(sizeof(int) * (argc) + 2);
...
int	push_swap_bis(t_args *args, char **argv, int argc)
{
	...
	if (ptr == NULL)
		return (0);
	argc = count_tab(ptr);
	if (verif1_bis(ptr) == 0 || verif2(argc, ptr) == 0)
